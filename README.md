...
> 이 코드는 실제 운영 중인 iOS 앱의 내부 설계와 아키텍처, 그리고 일부 주요 유스케이스와 테스트 전략을 _설명 목적으로 공개하는 것입니다. 실제 앱의 전체 소스코드나 구체적 상세 로직은 포함되어 있지 않습니다. 일부 코드는 샘플 또는 구조적 이해를 위한 예시이며, 실제 구현과 차이가 있을 수 있습니다.
...
### 헥사고날 아키텍처(포트 앤 아댑터)
헥사고날 아키텍처(Ports and Adapters)를 채택하여 비즈니스 로직과 외부 의존성을 분리했습니다. 이는 특정 기술(Firebase, MapKit 등)에 대한 의존성을 최소화하고, 향후 확장이나 기술 교체를 용이하게 합니다. 주요 구성 요소는 다음과 같습니다:
...
* 헥사고날 도입에 따른 폴더 구조, 코드 구조의 복잡성 증가 등의 문제가 있었으며, 소규모 프로젝트 적용시 의도치 않은 복잡성 증가 문제가 발생할 수 있음을 인지하고 적용 해야 합니다.
* 헥사고날 아키텍처가 앱에 1:1 매칭 되지 않기때문에 타협해야할 부분이 있다는 점을 유의하고 유연하게 적용해야 합니다.
